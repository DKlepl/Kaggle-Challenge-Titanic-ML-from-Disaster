...
sum(data$Ticket_people[data$Family_size==1]>1) #127 not much but still might get better info than family size of 1
...
embark_data = data_all[c(-4,-9,-11,-13,-14,-15,-17,-18)]
...
embark_data$Embarked[is.na(embark_data$Embarked)]=embark_data$Predictions
embark_data$Embarked[embark_data$Embarked==3]="C"
embark_data=embark_data[c(1,9)]

data_all = merge(data_all,embark_data,by="PassengerId")
data_all$Embarked=data_all$Embarked.y
data_all$Embarked.y=NULL
data_all$Embarked.x=NULL
sum(is.na(data_all$Embarked))

write.csv(data_all,"cleaner_data_all.csv",row.names = F)
```
